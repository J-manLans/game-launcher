...
As mentioned earlier, having the client submit a task to the worker would have made the workers implementation more modular compared to existing solution. That way the worker also could have been completely decoupled from the client, only care about the task at hand without any knowledge of who is asking for the task to be carried out. On the other hand, if the need to abstract away the implementation from the client the proposed solution is a valid one. However, the direct aggregation relation between the client and worker could benefit the program if being removed. To do that one could let the client give it's randNum to the worker through the manager. Then have the worker notify the manager when the work is done who inturn return the updated number to the client and notify him with its reference to the clientLock. That would have enabled the design to align more with the model on the right in this UML diagram.

![UML over different approaches](./src/main/resources/OOP2_lab_1.png)


The use of Object locks in the existing solution is a little bit redundant because the locks are not shared among multiple entities. Each class only contains two synchronized blocks, one for waiting and one for notifying. In this case, it would have been perfectly fine to lock on this instead of using a separate Object lock. This would simplify the code, as there's minimal risk of blocking other unrelated operations, and the locking mechanism would still ensure thread safety.

However, for learning purposes it made sense experimenting with the functionalities of Object locks.